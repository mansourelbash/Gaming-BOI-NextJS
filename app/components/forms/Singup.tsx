...
const SingUp = () => {
  ...
  const onSubmit = async (data: z.infer<typeof signupSchema>) => {
    startTransition(async () => {
      ...
      if (data.avatar) {
        ...
        try {
          console.log('dddddeeeeeee',process.env.NEXT_PUBLIC_CLOUDINARY_URL)
  ...
  return (
    <MotionItem animate={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 30 }}>
      <MaxWidthWrapper customPadding="py-14" className="flex flex-col gap-4 items-center w-full bg-black/60 rounded-2xl border border-input">
        <Logo />
        <Form {...form}>
